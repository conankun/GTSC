...
def sweep_cut(Px, x, permEv):
	...
	for i in range(n-1):
		...
		for j in tempRowInd:
			if per[j] > i:
				tranS1[i + 1] += x[ind].dot(tempRow[j]) / v[ind]
			if per[j] < i:
				tranS2[i + 1] = tranS2[i + 1] - x[ind].dot(tempRow[j]) / v[ind]

		for j in tempColInd:
			if per[j] > i:
				tranS2[i + 1] += x[j].dot(tempCol[j]) / v[j]
			if per[j] < i:
				tranS1[i + 1] = tranS1[i + 1] - x[j].dot(tempCol[j]) / v[j]
	...
	return (cutPoint, cut, cut[cutPoint], np.array([PS1[cutPoint+1], PS2[cutPoint+1], PS3[cutPoint+1]]))